...
function apply(req, ast, cb) {
  ...
  function prg(cb) {
    var prg = program.toObject({all: true, recurse: false});
    delete prg.names;
  ...
  function getArgMapString(map) {
    ...
    for(k in map) {
      ...
      v = map[k];
      //console.dir(v);
      v.toObject({all: true, item: function(o) {
        //console.log('item callback');
        s += util.format(
          formats.key,
          k, o.constructor.name, circular.stringify(o))
        return o;
      }});
    }
    s += '}';
  ...
  function opt(cb) {
    var s = getArgMapString(opts);
    var js = util.format(formats.opt, s);
    funcbody.unshift(extract(esprima.parse('program.options(opt);')));
    cb(null, [esprima.parse(js)]);
  }
  ...
  function cmd(cb) {
    ...
    funcbody.unshift(extract(esprima.parse('program.commands(cmd);')));
    cb(null, [esprima.parse(js)]);
  }
